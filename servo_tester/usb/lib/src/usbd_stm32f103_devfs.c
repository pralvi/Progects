...
#define USBD_DP_PORT    GPIOA
#define USBD_DP_PIN     12
...
inline static void set_gpiox() {
    ...
    if (USBD_DP_PORT == GPIOA) {RCC->APB2ENR |= RCC_APB2ENR_IOPAEN; return;} else
...
bool ep_config(uint8_t ep, uint8_t eptype, uint16_t epsize) {
    volatile uint16_t *reg = EPR(ep); //endpoint control register
...
int32_t ep_read_size(uint8_t ep) {
    pma_table *tbl = EPT(ep);
    volatile uint16_t *reg = EPR(ep);
    ...
    case (USB_EP_RX_VALID | USB_EP_BULK | USB_EP_KIND):
        ...
        return (*reg & USB_EP_SWBUF_RX ? tbl->rx1 : tbl->rx0).cnt;
