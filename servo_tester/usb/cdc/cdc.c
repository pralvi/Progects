...
static const struct usb_device_descriptor device_desc = {
    ...
    .idVendor           = 0x0483, //ST Microelectronics Vendor Id
    .idProduct          = 0x5740,
...
static const struct cdc_config config_desc = {
    ...
    .data_eptx = {
        ...
        .bmAttributes           = USB_EPTYPE_BULK,
        .wMaxPacketSize         = CDC_TX_DATA_SZ,
        .bInterval              = 0x01,
    },
};

static const struct usb_string_descriptor lang_desc     = USB_LANGID;
static const struct usb_string_descriptor manuf_desc = USB_MANUF_DESC;
static const struct usb_string_descriptor prod_desc  = USB_PRODUCT_DESC;
...
int32_t cdc_read_size() {
    return (USBIsAvailable()) ? usbd_ep_read_size(&udev, CDC_RXD_EP) : 0;
}

int32_t cdc_read(void *buf, uint16_t blen) {
    return (USBIsAvailable()) ? usbd_ep_read(&udev, CDC_RXD_EP, buf, blen) : 0;
}

int32_t cdc_write(void *buf, uint16_t blen) {
    return (USBIsAvailable()) ? usbd_ep_write(&udev, CDC_TXD_EP, buf, blen) : 0;
}
...
//        usbd_ep_config(dev, CDC_TXD_EP, USB_EPTYPE_BULK | USB_EPTYPE_DBLBUF, CDC_DATA_SZ);
        usbd_ep_config(dev, CDC_RXD_EP, USB_EPTYPE_BULK | USB_EPTYPE_DBLBUF, CDC_RX_DATA_SZ);
        usbd_ep_config(dev, CDC_TXD_EP, USB_EPTYPE_BULK | USB_EPTYPE_DBLBUF, CDC_TX_DATA_SZ);
...
static void cdc_init_usbd(usbd_device *dev) {
    usbd_init(dev, &usbd_hw, CDC_EP0_SIZE, ubuf, sizeof(ubuf));
    usbd_reg_config(dev, cdc_setconf);
    usbd_reg_control(dev, cdc_control);
    usbd_reg_descr(dev, cdc_getdesc);
}

void RestartCDC() {
    //TODO: reimplement using macro definitions
    //Force usb reenumeration
    usbd_enable(&udev, false);
    GPIOA->CRH = (GPIOA->CRH & ~(0x0F << (4 * (12/*USBD_DP_PIN*/ - 8)))) | (0x02 << (4 * (12/*USBD_DP_PIN*/ - 8)));
    GPIOA->BSRR = (0x0001 << 12/*USBD_DP_PIN*/ << 16);
    uint32_t i = 100000;
    while(--i) __asm("NOP");//Wait ~10mS
...
uint8_t USBIsAvailable() {
    return udev.status.device_state == usbd_state_configured && udev.status.last_event_type != usbd_evt_error && udev.status.last_event_type != usbd_evt_susp;
}
