...
#define __builtin_offsetof(type, member) (int)(&((type *)0)->member)
...
typedef struct {
    ...
    uint8_t     last_event_type; /**<\brief Type of the last proccessed event.*/
} usbd_status;
...
/**\brief Returns size of the data available to read from OUT or control endpoint
 * \param ep endpoint index, should belong to OUT or CONTROL endpoint.
 * \return size of the data, available to read.
 */
typedef int32_t (*usbd_hw_ep_read_size)(uint8_t ep);
...
struct usbd_driver {
    ...
    usbd_hw_ep_read_size    ep_read_size;       /**<\copybrief usbd_hw_ep_read_size */
...
/**\brief Read data available to read from endpoint
 * \param dev dev usb device \ref _usbd_device
 * \copydetails usbd_hw_ep_read
 */
inline static int32_t usbd_ep_read_size(usbd_device *dev, uint8_t ep) {
    return dev->driver->ep_read_size(ep);
}
