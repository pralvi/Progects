...
void utoaLTRCB(uint32_t value, void CB(char)) {
    unsigned char buf[11];
    unsigned char *pos = utoa(value, buf);
...
void itoaLTRCB(int32_t value, void CB(char)) {
    unsigned char buf[11];
    unsigned char *pos = itoa(value, buf);
...
void utoHexCB(uint32_t value, void CB(char)) {
    ...
    if (value) {
        uint8_t leading = 1;
        for(int i = 0; i < 8; i++) {
            unsigned char digit = value >> 28;
            if (leading == 0 || digit != 0) {
                CB(digit + (digit < 10 ? 0x30 : 0x37));
                leading = 0;
            }
            value <<= 4;
        }
...
char * ftoa(float value, char *result)
{
	...
	if(exp10 != 0)
	{
		*out_ptr++ = 'e';
		...
		char *powPtr = (char*)utoa(upow10, (unsigned char*)buffer);
