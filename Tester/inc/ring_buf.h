...
#define CONCAT1(First, Second) CONCAT2(First, Second)
#define STATIC_ASSERT(expr) typedef char CONCAT1(static_assert_failed_at_line_, __LINE__) [(expr) ? 1 : -1]

#define RBUF_PTR_TYPE char
...
struct buf_name { \
   volatile RBUF_PTR_TYPE wCnt; \
   volatile RBUF_PTR_TYPE rCnt; \
   RBUF_TYPE buf[buf_size]; \
}; \
STATIC_ASSERT(buf_size <= 256); \
STATIC_ASSERT((buf_size&(buf_size-1))==0 && buf_size > 1); \

#define RBUF_IS_EMPTY(rbuf) (rbuf.wCnt == rbuf.rCnt)
#define RBUF_IS_FULL(rbuf) (((RBUF_PTR_TYPE)(rbuf.wCnt - rbuf.rCnt) & ~(RBUF_PTR_TYPE)(sizeof(rbuf.buf)-1)) != 0)

#define RBUF_COUNT(rbuf) ((RBUF_PTR_TYPE)(rbuf.wCnt - rbuf.rCnt) & (RBUF_PTR_TYPE)(sizeof(rbuf.buf)-1))
#define RBUF_SIZE(rbuf) (RBUF_PTR_TYPE)sizeof(rbuf.buf)

#define RBUF_WRITE(rbuf, value) do { rbuf.buf[rbuf.wCnt & (RBUF_PTR_TYPE)(sizeof(rbuf.buf)-1)] = (value); rbuf.wCnt++; } while(0)
#define RBUF_READ(rbuf, dst) do { dst = rbuf.buf[rbuf.rCnt & (RBUF_PTR_TYPE)(sizeof(rbuf.buf)-1)]; rbuf.rCnt++; } while(0)
